...
config.color_scheme = "rose-pine-moon" -- rose-pine-moon
...
-- Cmd never reaches the shell on macOS, so translate the Mac line-editing
-- shortcut into the control code the shell already understands.
config.keys = {
	{ key = "Backspace", mods = "CMD", action = wezterm.action.SendKey({ key = "u", mods = "CTRL" }) },
	-- Home/End rather than Ctrl+A/Ctrl+E: Ctrl+A is the tmux prefix and would
	-- be swallowed before the shell ever saw it.
	...
	-- Esc-b / Esc-f are backward-word / forward-word in zsh's default keymap,
	-- so word motion needs no shell config at all.
	{ key = "LeftArrow", mods = "OPT", action = wezterm.action.SendString("\x1bb") },
	{ key = "RightArrow", mods = "OPT", action = wezterm.action.SendString("\x1bf") },
	-- Shift+Enter is indistinguishable from Enter on the wire, so send the
	-- Esc-prefixed newline that Claude Code and friends read as "soft newline".
